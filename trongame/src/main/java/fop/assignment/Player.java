...
public class Player extends GameCharacter {
    ...
    // Getter and Setter for the Hero Name 
    ...
    public void addXP(int amount) {
        ...
        while (true) {
            // -STOP IF WE HIT MAX LEVEL DURING LOOP 
            if (level >= MAX_LEVEL) {
                break; 
            }
            ...
            // when enough XP, level up and subtract the cost
            if (this.xp >= xpNeeded) {
                this.xp -= xpNeeded;
                levelUp();
            } else {
    ...
    public void levelUp() {
        ...
        // Calculate Max Life: Base is 3.0. +1 Max Life for every 10 levels
        double maxLives = 3.0 + (int)(level / 10);
        
        // Set current lives to that maximum
        this.lives = maxLives;
        ...
        if (level % 15 == 0) {
            this.maxDiscSlots++;
            this.currentDiscSlots++; // Fill new slot immediately
